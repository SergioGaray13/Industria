    ...
    const fetchUserRoleAndFAQs = async () => {
      try {
        ...
        if (userError) {
          ...
          // No bloqueamos la carga de FAQs por este error
        }

        setUserRole(userData?.role || null)

        // Primero, verificar si la tabla existe y está accesible
        ...
        if (testError) {
          ...
          if (testError.message.includes('relation "public.faq" does not exist')) {
            setError('La tabla de preguntas frecuentes no existe. Contacta al administrador.')
          } else if (testError.message.includes('permission denied')) {
            setError('No tienes permisos para acceder a las preguntas frecuentes.')
          } else {
        ...
        if (faqError) {
          console.error('Error al cargar FAQs:', {
          ...
          console.log('FAQs cargadas exitosamente:', faqData?.length || 0, 'registros')
          setFaqs(faqData || [])
        }
      } catch (error) {
